...
export default async function RoutinesPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {routines.length === 0 ? (
        <p className="text-sm text-[#888888]">Todavía no creaste rutinas.</p>
      ) : (
          ...
          {routines.map((r, i) => (
            <FadeIn key={r.id} delay={Math.min(i * 0.05, 0.4)}>
              <Link
                ...
                className="flex items-center justify-between rounded-2xl border border-[#1e1e1e] bg-[#111111] p-4 transition-colors duration-300 hover:border-[#e8001c]"
              >
                <div>
